...
public abstract class AbstractAbapCodegen extends DefaultCodegen implements CodegenConfig {

    protected static Logger LOGGER = LoggerFactory.getLogger(AbstractAbapCodegen.class);

    protected String packageName = "swagger";

    public AbstractAbapCodegen() {
        super();
        ...
        typeMapping.put("string", "STRING");//ok
        typeMapping.put("UUID", "STRING");//ok
        typeMapping.put("date", "DATUM");//ok
        typeMapping.put("DateTime", "STRING");
        typeMapping.put("password", "STRING");
        typeMapping.put("File", "XSTRING");
        typeMapping.put("file", "XSTRING");
        // map binary to string as a workaround
        // the correct solution is to use []byte
        typeMapping.put("binary", "string");
        typeMapping.put("ByteArray", "string");
        typeMapping.put("object", "");
        ...
        cliOptions.clear();
        cliOptions.add(new CliOption(CodegenConstants.PACKAGE_NAME, "Abap package name (convention: lowercase).")
    ...
    public String escapeReservedWord(String name)
    {
        ...
        // UppercaseLetter so that Abap treats them as public/visible.
    ...
    public String toModelName(String name) {
        ...
        LOGGER.warn("toModelName" + name ) ;
        return camelize(toModel(name));
    }

    @Override
    public String toModelFilename(String name) {
        LOGGER.warn("toModelFilename" + name ) ;
      return toModel( name);
    }

    public String toModel(String name) {
        LOGGER.warn("toModel" + name ) ;
        ...
        if (isReservedWord(name)) {
            LOGGER.warn(name + " (reserved word) cannot be used as model name. Renamed to " + ("zcl_" + name));
            name =  name; // e.g. return => ModelReturn (after camelize)
        }

        // model name starts with number
        if (name.matches("^\\d.*")) {
            LOGGER.warn(name + " (model name starts with number) cannot be used as model name. Renamed to " + ("zcl_" + name));
            name = name; // e.g. 200Response => Model200Response (after camelize)
        }
